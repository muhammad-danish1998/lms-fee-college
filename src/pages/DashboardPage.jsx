...
export function DashboardPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setError('Failed to connect to the database. Please verify network and Supabase connection.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Are you sure you want to permanently delete student "${name}"? All related payments and records will also be deleted.`)) {
      return;
    }
    try {
  ...
  const totalFeeSum = students.reduce((acc, s) => acc + (Number(s.total_fee) || 0), 0);
  const totalPaidSum = students.reduce((acc, s) => acc + (Number(s.total_paid) || 0), 0);
  const totalDuesSum = students.reduce((acc, s) => acc + (Number(s.dues) || 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">Dashboard</h2>
          <p className="text-xs md:text-sm text-slate-400">
            Overview of student enrollments, fee collections, and verification progress
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 relative overflow-hidden shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-semibold uppercase text-slate-400">Enrolled</span>
            <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-blue-500/10 text-blue-400 flex items-center justify-center border border-blue-500/20">
              <Users className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            </div>
          </div>
          <div className="mt-2 sm:mt-3">
            <div className="text-xl sm:text-2xl font-bold text-white">{totalStudentsCount}</div>
            <div className="text-[10px] sm:text-xs text-slate-400 mt-0.5">Active records</div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 relative overflow-hidden shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-semibold uppercase text-slate-400">Total Fee</span>
            <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-indigo-500/10 text-indigo-400 flex items-center justify-center border border-indigo-500/20">
              <DollarSign className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            </div>
          </div>
          <div className="mt-2 sm:mt-3">
            <div className="text-xl sm:text-2xl font-bold text-white">{formatCurrency(totalFeeSum)}</div>
            <div className="text-[10px] sm:text-xs text-slate-400 mt-0.5">Committed gross</div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 relative overflow-hidden shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-semibold uppercase text-slate-400">Collected</span>
            <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-emerald-500/10 text-emerald-400 flex items-center justify-center border border-emerald-500/20">
              <CheckCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            </div>
          </div>
          <div className="mt-2 sm:mt-3">
            <div className="text-xl sm:text-2xl font-bold text-emerald-400">{formatCurrency(totalPaidSum)}</div>
            <div className="text-[10px] sm:text-xs text-slate-400 mt-0.5">Verified receipts</div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 relative overflow-hidden shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-xs font-semibold uppercase text-slate-400">Remaining</span>
            <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-amber-500/10 text-amber-400 flex items-center justify-center border border-amber-500/20">
              <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            </div>
          </div>
          <div className="mt-2 sm:mt-3">
            <div className="text-xl sm:text-2xl font-bold text-amber-400">{formatCurrency(totalDuesSum)}</div>
            <div className="text-[10px] sm:text-xs text-rose-400 mt-0.5 flex items-center gap-1 font-medium">
              {overdueCount > 0 && <AlertTriangle className="w-3 h-3 shrink-0" />}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="px-4 sm:px-6 py-3.5 sm:py-4 border-b border-slate-800 flex items-center justify-between">
          <div>
            <h3 className="text-sm sm:text-base font-bold text-white">Student Enrollment &amp; Fee Records</h3>
            <p className="text-[11px] sm:text-xs text-slate-400">Verified transaction ledger &amp; progress stages</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-400 space-y-3">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              No student records match your current filters. Try changing your search query or enroll a new student.
            </p>
            <Link
              to="/enroll"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-teal-300 text-xs font-semibold transition-colors"
            >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-950/60 border-b border-slate-800 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                    ...
                    <th className="py-3.5 px-4">Remaining Dues</th>
